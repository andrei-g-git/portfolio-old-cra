...
var panzoom = function panzoom(selector) {
  ...
  var pan = options.pan !== false;
  var zoom = options.zoom !== false; // Default: true	

  var bound = ['inner', 'outer', 'none'].includes(options.bound) ? options.bound : 'inner';
  var wheel_step = options.wheel_step > 0.01 && options.wheel_step < 4 ? options.wheel_step : 0.2;
  var scale_min = options.scale_min > 0.01 && options.scale_min < 20 ? options.scale_min : 0.01;
  var scale_max = options.scale_max > 0.01 && options.scale_max < 20 ? options.scale_max > scale_min ? options.scale_max : scale_min : 10; // For panning (translate)

  var lastPosX, lastPosY; // Needed because of decimals 

  var posX_min, posY_min, posX_max, posY_max;
  var parentScale; // Needed for avoid calculate every pointermove

  var vvpScale, dprScale; // Needed to take into account e.movementX in touch screens
  // For touching devies

  var lastTouchX, lastTouchY; // To calculate delta position when moving fingers

  var isPinching = false;
  var isTouching = false;
  var pinch_dist1; // Attach event listeners

  document.querySelectorAll(':scope ' + selector).forEach(function (elem) {
    var isValid = normalize(elem);
    if (!isValid) return;

    if (zoom) {
      elem.addEventListener("wheel", handle_wheel, {
        passive: false
      });
    }
    ...
    if (pan) {
      ...
      elem.addEventListener("touchend", handle_touchend); // Pointer events, needed for move
      //elem.addEventListener("pointerdown", handle_pointerdown);

      elem.addEventListener("pointerup", handle_pointerup);
      elem.addEventListener("pointermove", handle_pointermove); //elem.addEventListener("pointercancel", handle_pointerup);
  ...
  function normalize(elem) {
    ...
    var widthp = elem.parentNode.offsetWidth;
    var height = elem.offsetHeight;
    var heightp = elem.parentNode.offsetHeight;
    if (width > widthp) if (bound == "inner" && (width > widthp || height > heightp)) {
    ...
    */
    ...
    elem.style.backgroundSize = 'cover'; //elem.style.transition = 'all 0.1s ease 0s';
  ...
  function do_zoom(a, b, c, d) {} // function do_zoom(elem, deltaScale, offsetX, offsetY) {
  ...
  function handle_pointermove(e) {
    ...
    e.preventDefault();
    e.stopPropagation(); // Detect when the cursor exits parent node
    ...
    var deltaX = e.movementX / parentScale / dprScale; // vvpScale It's pinch default gesture zoom (Android). Ignore in Desktop

    var deltaY = e.movementY / parentScale / dprScale; // vvpScale It's pinch default gesture zoom (Android). Ignore in Desktop
  ...
  function handle_touchstart(e) {
    ...
    e.preventDefault();
    e.stopPropagation();
    isTouching = true; // Check if two fingers touched screen. If so, handle Zoom
    ...
    lastTouchY = e.touches[0].pageY; // It continues handling pointerdown event
  }

  function handle_touchmove(e) {
    if (e.target !== e.currentTarget) return; // Check if two fingers touched screen. If so, handle Zoom

    if (e.targetTouches.length == 2 && e.changedTouches.length == 2) {
      var distX = e.touches[0].pageX - e.touches[1].pageX;
      var distY = e.touches[0].pageY - e.touches[1].pageY;
      var pinch_dist2 = Math.hypot(distX, distY); //get rough estimation of new distance between fingers

      var deltaScale = (pinch_dist2 - pinch_dist1) / 50;
      ...
      var _e$target$getBounding = e.target.getBoundingClientRect(),
          ...
          height = _e$target$getBounding.height; //const matrix = new WebKitCSSMatrix(getComputedStyle(e.target).getPropertyValue("transform"));
      //const {a:scaleX, b:skewY, c:skewX, d:scaleY, e:translateX, f:translateY} = matrix;			


      var offsetX0 = (e.touches[0].clientX - x) / width * e.target.offsetWidth;
      var offsetY0 = (e.touches[0].clientY - y) / height * e.target.offsetHeight;
      var offsetX1 = (e.touches[1].clientX - x) / width * e.target.offsetWidth;
      var offsetY1 = (e.touches[1].clientY - y) / height * e.target.offsetHeight; //status.innerHTML = e.touches[0].pageX + ' '+x;

      var offsetX = offsetX0 + (offsetX1 - offsetX0) / 2;
      var offsetY = offsetY0 + (offsetY1 - offsetY0) / 2;
      do_zoom(e.target, deltaScale, offsetX, offsetY);
    } else if (e.targetTouches.length == 1 && !isPinching) {
      var deltaX = (e.touches[0].pageX - lastTouchX) / parentScale; // vvpScale It's pinch default gesture zoom (Android). Ignore in Desktop

      var deltaY = (e.touches[0].pageY - lastTouchY) / parentScale; //

      lastTouchX = e.touches[0].pageX;
      lastTouchY = e.touches[0].pageY; // Else, it is a drag. Handle with pointermouse event
  ...
  function handle_wheel(e) {
    ...
    e.stopPropagation();
    if (e.target !== e.currentTarget) return;
    var deltaScale = e.wheelDelta * wheel_step / 120;
